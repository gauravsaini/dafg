        ...
        if cmd.action == Action.ACCEPT_VERDICT:
            ...
            if not active_dispatch:
                record = AuditRecord(
                    ...
                    action=cmd.action.value,
                    node_id=cmd.node_id,
                    reason="No active dispatch recorded for node; cannot accept verdict",
                )
                return [], record

            if not cmd.dispatch_identity:
                record = AuditRecord(
                    timestamp=datetime.now(timezone.utc).isoformat(),
                    idempotency_key=cmd.idempotency_key,
                    action=cmd.action.value,
                    node_id=cmd.node_id,
                    reason="Missing dispatch_identity on verdict proposal",
                )
                return [], record

            # Exact agreement on dispatch identity
            if (
